...
public class MovieView {
    ...
    public ObservableList<Movie> getAllMovies() {
        return allMovies; // Возвращаем все фильмы
    }

    public ObservableList<Movie> getFavoriteMovies() {
        return favoriteMovies; // Возвращаем избранные фильмы
    }
    ...
    public void removeFromFavorites(String title) {
        ...
        if (movie != null) {
            ...
            allMovies.add(movie); // Возвращаем фильм в список всех фильмов
        }
    }
    ...
    public void updateMovieList(List<MovieComponent> movies) {
        allMovies.clear(); // Очистка текущего списка
        for (MovieComponent movie : movies) {
            if (movie instanceof Movie) {
                allMovies.add((Movie) movie); // Добавление фильмов в список
            }
        }
    }
}